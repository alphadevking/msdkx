 ...
 */
export function convertDurationToUTCTimestamp(
  durationStr: string,
  baseDate?: Date
): DurationResult {
  ...
  const unitAdders: { [unit: string]: (d: Date, value: number) => void; } = {
 ...
 */
export function convertUTCTimestampToDurationString(
  utcTimestamp: number,
  baseDate?: Date
): string {
  ...
  const addUnitFunctions: { [unit: string]: (d: Date, amount: number) => Date; } = {
    y: (d, amount) => {
  ...
  function countUnits(
    start: Date,
    target: Date,
    unit: keyof typeof addUnitFunctions
  ): { count: number; newDate: Date; } {
    let count = 0;
  ...
  const order: (keyof typeof addUnitFunctions)[] = [
    'y',
    'mo',
    'w',
    'd',
  ...
  const results: Partial<Record<keyof typeof addUnitFunctions, number>> = {};
 ...
 */
export function convertDurationToNumbers(durationStr: string): {
  totalSeconds: number;
  totalMilliseconds: number;
} {
  // Define conversion factors (using common approximations)
  const factorsInSeconds: { [unit: string]: number; } = {
    y: 365 * 24 * 3600,      // 1 year = 365 days
    mo: 30 * 24 * 3600,      // 1 month = 30 days
    w: 7 * 24 * 3600,        // 1 week = 7 days
